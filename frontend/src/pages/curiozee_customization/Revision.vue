	...
	<div>
		...
		<div class="flex min-h-0 w-full flex-1 flex-col p-5 pb-10">
			...
			<div class="border-b pb-5 mb-6">
				<h2 class="text-2xl font-semibold text-ink-gray-9 leading-tight">
			...
			<!-- Status Filter Tabs (placed above grid to align columns on the same line) -->
			<div v-if="!revisionStore.revisionFetcher.loading || recommendations.length" class="mb-6 flex">
				<TabButtons
					v-model="activeTab"
					:buttons="tabButtons"
				/>
			...
			<div
				v-if="revisionStore.revisionFetcher.loading && !recommendations.length"
				class="flex justify-center py-20"
			>
				<Spinner class="w-6 h-6 text-ink-gray-4" />
			</div>

			<!-- Two-column workspace: matching Statistics / Dashboard page -->
			<div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start w-full">
				...
				<div class="space-y-6">
					...
					<div v-else class="text-center py-20 border border-outline-gray-2 rounded-xl bg-surface-white px-6">
						...
						<h3 class="font-semibold text-base text-ink-gray-8 mb-1">{{ emptyStateTitle }}</h3>
				...
				<div class="space-y-6">
					...
					<div class="bg-surface-white border border-outline-gray-2 p-5 rounded-xl shadow-sm space-y-2">
						...
						<div class="flex items-baseline gap-2">
							<span class="text-2xl font-extrabold text-ink-gray-9 leading-none">{{ pendingCount }}</span>
...
const recommendations = computed(() => revisionStore.recommendations || [])

const pendingCount = computed(() => {
	return recommendations.value.filter(r => ['Pending', 'Viewed'].includes(r.status)).length
})
...
const emptyStateTitle = computed(() => {
	if (activeTab.value === 'Completed') return __('No completed revisions yet.')
	if (activeTab.value === 'Dismissed') return __('No dismissed recommendations.')
	return __('All caught up!')
})

const emptyStateSubtitle = computed(() => {
	if (activeTab.value === 'Completed')
		return __('Complete lessons to see them reflected here automatically.')
	return __('No recommendations match this status. Keep studying new lessons!')
})
